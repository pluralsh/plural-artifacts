locals {
    ...
    private_subnet_ids        = var.create_cluster ? one(module.vpc[*].private_subnets_ids) : var.private_subnet_ids
    public_subnet_ids         = var.create_cluster ? one(module.vpc[*].public_subnets_ids) : var.public_subnet_ids
    worker_private_subnet_ids = var.create_cluster ? one(module.vpc[*].worker_private_subnets_ids) : var.worker_private_subnet_ids
    vpc_id                    = var.create_cluster ? one(module.vpc[*].vpc_id) : data.aws_eks_cluster.cluster[0].vpc_config[0].vpc_id
